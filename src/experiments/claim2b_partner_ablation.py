...
Design:
  1. Train ONE multinomial logistic head on frozen features under the P1
     (image-level) split's train fold.
...
leakage. This script does BOTH of the plan's suggested remedies:
...
Also reports: a bootstrap CI on the difference (resampling PATIENTS
independently within each group, not images) and a label-shuffle
...
Writes results/claim2b_partner_ablation.json and figures/figure3_claim2b_partner_ablation.png.
...
from src.experiments.claim2_protocols import load_active_manifest, build_xy, fit_and_eval
...
def per_grade_breakdown(y_true, y_pred, min_n=10):
    out = {}
    for g in range(5):
        mask = y_true == g
        n_g = int(mask.sum())
...
def permutation_test_group_diff(y_true, y_pred, is_present, n_perm=2000, seed=42):
    ...
    QWK difference each time -- null distribution for 'does the
    present/absent split explain a real difference, or would any random
    split of this size produce one just as large?'"""
    rng = np.random.RandomState(seed)
    observed = qwk(y_true[is_present], y_pred[is_present]) - qwk(y_true[~is_present], y_pred[~is_present])
    null_diffs = np.empty(n_perm)
    n_present = int(is_present.sum())
    n = len(is_present)
...
def main():
    ...
    # ---- train ONE model on P1 train, evaluate on the full P1 test set ----
    ...
    _, y_pred = fit_and_eval(X_train, y_train, X_test, y_test, seed=args.seed, bootstrap_train=False)
    ...
    print(f"\n--- RAW QWK (same model, same weights) ---")
    ...
    # ---- grade distributions + stratified per-grade breakdown -------------
    ...
    print("\n--- GRADE DISTRIBUTIONS (why matching matters) ---")
    print(pd.DataFrame({"present": grade_dist_p, "absent": grade_dist_a}).round(3).to_string())

    breakdown_p = per_grade_breakdown(y_true_p, y_pred_p)
    breakdown_a = per_grade_breakdown(y_true_a, y_pred_a)
    print("\n--- PER-GRADE ACCURACY, STRATIFIED ---")
    for g in range(5):
    ...
    # ---- grade-matched resample: subsample the LARGER group to match the smaller ----
    ...
    matched_larger_true, matched_larger_pred = larger_true[sel_idx], larger_pred[sel_idx]
    qwk_matched_larger = qwk(matched_larger_true, matched_larger_pred)
    ...
    print(f"\n--- GRADE-MATCHED COMPARISON (subsampled '{larger_name}' down to match '{smaller_name}'"
          f"'s per-grade counts) ---")
    if shortfall:
        print(f"  NOTE: shortfall (larger group didn't have enough at these grades): {shortfall}")
    print(f"  matched partner-present QWK: {qwk_matched_present:.4f}")
    print(f"  matched partner-absent QWK:  {qwk_matched_absent:.4f}")
    print(f"  matched difference = {qwk_matched_present - qwk_matched_absent:+.4f}  "
          f"(raw difference was {qwk_present - qwk_absent:+.4f})")

    # ---- bootstrap CI on the raw difference, permutation test --------------
    ...
    print(f"\n--- BOOTSTRAP CI on (present - absent), resampling patients within each group ---")
    print(f"  95% CI: [{ci_lo:.4f}, {ci_hi:.4f}]")

    perm_result = permutation_test_group_diff(y_test, y_pred, is_present, n_perm=args.n_permutations, seed=args.seed)
    print(f"\n--- PERMUTATION TEST (shuffle present/absent label, {args.n_permutations} shuffles) ---")
    print(f"  observed diff = {perm_result['observed_diff']:+.4f}   "
          f"null mean = {perm_result['null_mean']:+.4f} (std {perm_result['null_std']:.4f})   "
          f"p-value = {perm_result['p_value_two_sided']:.4g}")
    ...
    if ci_lo > 0 and perm_result["p_value_two_sided"] < 0.05:
        ...
        interpretation = ("No significant difference detected (CI includes 0, or p>=0.05) -- report "
                           "honestly as a negative/null result on this ablation.")
    print(f"\n--- INTERPRETATION ---\n{interpretation}")
    ...
    present_vals = [qwk_present, qwk_matched_present]
    absent_vals = [qwk_absent, qwk_matched_absent]
    ...
    ax2.set_ylabel("QWK")
    ax2.set_ylim(0, 1)
    ax2.set_title(f"Same model, same weights, evaluated on each group\n"
                  f"diff 95% CI [{ci_lo:.3f}, {ci_hi:.3f}], permutation p={perm_result['p_value_two_sided']:.3g}")
