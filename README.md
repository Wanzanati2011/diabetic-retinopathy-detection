...
| Does grading both eyes together beat grading them separately? | **Yes, but most of the gain is the classifier head, not the eye-fusion.** Total gain (pool+ordinal vs. per-eye-max+multinomial, 384px): +0.0725 QWK (CI [+0.046, +0.098], excludes 0). Decomposed: the ordinal-vs-multinomial **head effect** alone is +0.0481 (CI [+0.022, +0.074]); the **fusion effect** (averaging both eyes' features) adds a further +0.0244 (CI [+0.004, +0.042]) — real and significant, but the smaller of the two, and it reversed sign on one of three tested backbones. | `results/claim3_decomposed_tf_efficientnet_b0_384.json` |
...
- The deployed checkpoint's test QWK (0.716) sits in this project's own "correct, proceed" acceptance band, but the retrain that got it there also dropped grade-1 (Mild NPDR) recall substantially (0.461→0.161). **Investigated, not fixed:** four decision rules were tested against a pre-registered ≥0.30 recall bar and none clear it (best: 0.228) — verdict `H2_representation_ceiling`, i.e. the microaneurysm evidence does not survive this project's 384px representation at any operating point. See `results/grade1_diagnosis.json` and `app/release/MODEL_CARD.md`.
...
- **Clinical operating point** (`results/calibration_reject.json`, `app/release/thresholds.json`): temperature T=3.3674 fitted on validation only (test ECE 0.1628→0.0285); referral at `P(grade≥2) ≥ 0.135` gives sensitivity 89.6% / specificity 69.7% on test; an uncertainty gate (max calibrated prob < 0.593) routes the least-confident ~20% to a human grader and lifts selective QWK from 0.716 to 0.774 on the rest (Acceptance Test 11.1: pass). Applied in the app via `app/core/decision.py`, with a visible fallback to raw uncalibrated confidence if the checkpoint hash or acceptance verdict ever don't check out.
